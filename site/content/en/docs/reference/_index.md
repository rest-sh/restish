---
title: Reference
linkTitle: Reference
weight: 50
description: Factual, scannable reference pages for commands, config, output, and plugins.
---

Reference pages are optimized for scanning, linking, and answering factual
questions quickly.

## Main References

- [Commands](./commands/) for the CLI surface area and command families.
- [Example API](./example-api/) for the canonical `api.rest.sh` resources used in the docs.
- [Global Flags](./global-flags/) for shared request, output, and TLS options.
- [Environment Variables](./environment-variables/) for shell-level defaults and path overrides.
- [API Management](./api-management/) for `restish api`, cache, cert, and auth helpers.
- [Generic HTTP Commands](./http-commands/) for the built-in HTTP verb commands and bare-URL shortcut.
- [Edit Command](./edit-command/) for fetch-edit-update workflows.
- [Links Command](./links-command/) for normalized link inspection.
- [Cache Commands](./cache-command/) for cache inspection and clearing.
- [Cert Command](./cert-command/) for TLS certificate inspection.
- [Auth Header Command](./auth-header-command/) for auth debugging.
- [Setup Command](./setup-command/) for shell input protection.
- [Plugin Commands](./plugin-command/) for plugin install/remove/debug helpers.
- [Config](./config/) for config file structure and profile fields.
- [Profiles](./profiles/) for per-environment overrides and profile selection.
- [Shorthand Syntax](./shorthand/) for structured request-body input on the command line.
- [Query Syntax](./query-syntax/) for shorthand vs jq filter behavior.
- [Content Types](./content-types/) for request encoding and response decoding support.
- [Output Defaults](./output-defaults/) for TTY-aware rendering and redirected output behavior.
- [Output Formats](./output-formats/) for formatter behavior and use cases.
- [Plugins](./plugins/) for plugin architecture and operator-facing commands.
- [Plugin Manifest](./plugin-manifest/) for plugin metadata fields.
- [Plugin Messages](./plugin-messages/) for hook, command, and TLS-signer protocol behavior.

## How To Use This Section

Start in a guide when you want examples and explanation. Jump here when you
already know the workflow and need exact names, flags, or fields.
