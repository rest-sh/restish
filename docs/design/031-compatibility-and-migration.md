...
- removal of the v1/v2-draft API-or-URI, Authorization-header-only auth inspect
  ...
  and `restish api auth get <api>` for curl-friendly script fragments, because
  v2 auth can be credential-specific and may not use the `Authorization` header
...
- redirected unfiltered downloads preserve the original response body bytes
