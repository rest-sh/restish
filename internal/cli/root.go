...
func (c *CLI) addGlobalFlags(root *cobra.Command) {
	...
	pf.StringP("rsh-output-format", "o", "", "Output format: "+output.FormatterNames(c.formatters)+" (default: readable on TTY, JSON for structured non-TTY output; use -r for raw output; see --rsh-columns, --rsh-sort-by for table)")
	...
	pf.BoolP("rsh-raw", "r", false, "Raw output: original body bytes, or unquoted filtered values")
