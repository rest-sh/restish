...
func (c *CLI) addGlobalFlags(root *cobra.Command) {
	...
	pf.StringP("rsh-output-format", "o", "", "Output format: "+output.FormatterNames(c.formatters)+" (default: readable on TTY, JSON for structured non-TTY output; use -o raw for original bytes; see --rsh-columns, --rsh-sort-by for table)")
